...
function! OpenUnityCodeExamples()
    call feedkeys(":e c:/myhome/UnityCodeExamples.cs\<cr>")
endfunction

function! OpenGodotCodeExamples()
    call feedkeys(":e c:/myhome/GodotCodeExamples.gd\<cr>")
endfunction
...
function! BuildAndRun()
    ...
    execute ("w")
	if l:current_filetype == "cs"
		execute("normal! \<c-w>100j")
		let l:curr_bufname = bufname()
		let l:cmd_buf_name = "!C:\\Windows\\system32\\cmd.exe"
		if l:curr_bufname == l:cmd_buf_name
			call feedkeys("BuildAndRun.Bat\<cr>")
			call feedkeys("\<c-w>p")
		else 
			echom 'Run() not cmd buffer'
		endif
	endif
endfunction

function! BuildAndRunUnitTest()
	let l:current_filetype = &filetype
	execute ("w")
	if l:current_filetype == "cs"
		"feedkeys("\<c-w>100j")
		...
		let l:cmd_buf_name = "!C:\\Windows\\system32\\cmd.exe"
		if l:curr_bufname == l:cmd_buf_name
			call feedkeys("BuildAndRunUnitTest.Bat\<cr>")
			call feedkeys("\<c-w>p")
		endif
	endif
endfunction
...
" key mapping
        ...
        " key mapping
                ...
                map <F11> :call OpenCodeSnippet()<cr>
